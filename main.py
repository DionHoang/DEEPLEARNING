...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="phobert",
        choices=["phobert", "phobert-lora", "lstm", "bilstm"],
    ...
    batch_size = args.batch_size or (
        BATCH_SIZE if "phobert" in args.model else BATCH_SIZE
    )
    epochs = args.epochs or (EPOCHS if "phobert" in args.model else LSTM_EPOCHS)
    learning_rate = args.lr or (
        LEARNING_RATE if "phobert" in args.model else LSTM_LEARNING_RATE
    )
    ...
    if args.mode == "train":
        ...
        logger.info(
            f"Training completed successfully. Best validation loss: {results['best_val']:.4f}"
        )
    ...
    # 2. Mode: EVALUATE
        ...
        if not chk_path:
            chk_name = (
                "student_distilled.pt" if "distilled" in args.model else "model.pt"
            )
        ...
        if not chk_path or not os.path.exists(chk_path):
            logger.warning(
                f"No checkpoint found matching {chk_path or 'default'}. Running evaluation with random weights!"
            )
        else:
        ...
        token_report, entity_report, flat_preds, flat_labels, all_preds, all_labels = (
            run_evaluation(
                model,
                test_loader,
                tokenizer,
        ...
        for etype, metrics in entity_report["per_type"].items():
            logger.info(
                f"  {etype:<25} | P: {metrics['precision']:.4f} | R: {metrics['recall']:.4f} | F1: {metrics['f1-score']:.4f} | Support: {metrics['support']}"
            )
        ...
        plot_confusion_matrix(
            ...
            flat_labels,
            LABEL_LIST,
            os.path.join(PLOT_DIR, f"{args.model}_confusion_matrix.png"),
        )
        plot_entity_distribution(
            flat_labels, LABEL_LIST, os.path.join(PLOT_DIR, "entity_distribution.png")
        )
    ...
    # 4. Mode: DISTILL
        ...
        logger.info(
            f"Distillation finished. Best Student checkpoint saved at: {results['best_path']}"
        )
    ...
    # 5. Mode: QUANTIZE
        ...
        if not chk_path or not os.path.exists(chk_path):
            logger.error(
                "Error: Quantization requires a trained baseline model checkpoint."
            )
            return
        ...
        logger.info(
            f"Quantized Model overall Accuracy on Val: {token_report['accuracy']:.4f}"
        )
        logger.info(
            f"Quantized Model overall Entity F1-Score on Val: {entity_report['overall']['f1-score']:.4f}"
        )
